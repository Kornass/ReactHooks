// Memoization recap - When a complex function stores its output so the next time it is called with the same input. It’s similar to caching, but on a more local level. It can skip any complex computations and return the output faster as it’s already calculated. This can have a significant effect on memory allocation and performance, and that strain is what the useCallback hook is meant to alleviate.

//  UseCallback pairs nicely with another hook called useMemo. The key difference is that useMemo returns a memoized value, whereas useCallback returns a memoized function. That means that useMemo is used for storing a computed value, while useCallback returns a function that you can call later on.Example:
...
// Why to use that?
// When React re-renders your component, it also recreates the functions you’ve declared inside your component. When React re-renders your component, it will see any functions that are declared in your component as being new functions.
//This is fine most of the time, and simple functions are easy to compute and will not impact performance. But the other times when you don’t want the function to be seen as a new function, you can rely on useCallback to help you out. Cases for UseCallback:
...
// Your function is a dependency of another hook, like useEffect
